...
func (p *Pool) GetGlobalID(ctx context.Context) (int64, error) {
	ctx, span := p.tracer.Start(ctx, "Pool.GetGlobalID")
	defer span.End()
	...
	span.SetAttributes(attribute.Int64("global_id", id))
...
func (p *Pool) GetBatchGlobalID(ctx context.Context, batchSize int) ([]int64, error) {
	ctx, span := p.tracer.Start(ctx, "Pool.GetBatchGlobalID")
	defer span.End()
...
func (p *Pool) fetchFromDB(ctx context.Context, count int) ([]int64, error) {
	...
	var startID int64
	err := p.db.QueryRowContext(ctx, "SELECT murex.get_next_batch_version_id($1)", count).Scan(&startID)
	if err != nil {
		span.RecordError(err)
		return nil, fmt.Errorf("failed to fetch GlobalIDs from database: %w", err)
	}
